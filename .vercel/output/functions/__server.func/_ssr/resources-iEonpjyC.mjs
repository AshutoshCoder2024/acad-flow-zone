...
import { a as adminDeleteResource, c as adminGetSignedUrl, r as adminCreateResource, s as adminFetchResources } from "./admin-api.functions-UHpZ8eZT.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/resources-iEonpjyC.js
...
function ResourcesPage() {
	...
	const query = useQuery({
		...
		queryFn: async () => {
			if (isEnvAdmin && adminToken) return adminFetchResources({ data: {
				token: adminToken,
				search,
				type,
				semester
	...
	async function handleCreate(form, file, typeValue) {
	...
	async function handleDelete(id, path) {
		...
		if (isEnvAdmin && adminToken) {
			try {
				await adminDeleteResource({ data: {
					token: adminToken,
					id,
					fileUrl: path
				} });
...
function ResourceForm({ onSubmit }) {
	...
	const [type, setType] = (0, import_react.useState)("notes");
